...
def parse_data(url):
    ...
    if response:
        ...
        for item in data['liveEventInfos']:
            if 'scoreFunction' in item and item['scoreFunction'] == 'Basketball' and len(item['scores']) > 1 and len(item['scores'][1]) == 3:
                ...
                if event and factor:
                    ...
                    if 1800 < item['timerSeconds'] < 2160:
                        score_predict = round(score_total / timer * 2880)
                    elif 1200 < item['timerSeconds'] < 1440:
                        score_predict = round(score_total / timer * 2400)
                    elif 1440 < item['timerSeconds'] < 1800:
